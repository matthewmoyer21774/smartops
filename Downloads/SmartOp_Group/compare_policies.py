...
def main():
    ...
    backtest_rows, backtest_demands, training_tail, backtest_dates = build_backtest_data(df)

    print(f"\nBacktest: {len(backtest_demands)} periods")
    print(f"Date range: {pd.Timestamp(backtest_dates[0]).strftime('%Y-%m-%d')} "
    ...
    print(f"  Model trained on rows: 0-{n - 200 - 1} (train+val, test_size=200 held out)")
    print(f"  Backtest rows {bt_start}-{n-1} are within the 200-row held-out test set: OK")
    ...
    for rank, (name, r) in enumerate(sorted_results, 1):
        ...
        print(f"  {name:<35} {r['total_cost']:>8.0f} {r['holding']:>8.0f} "
    ...
    if worst_r['total_cost'] > 0:
        print(f"  Savings: {worst_r['total_cost'] - best_r['total_cost']:.0f} "
